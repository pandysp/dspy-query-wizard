...
async def test_train_process():
    ...
    # Mock dependencies
    with (
        patch("backend.train.BootstrapFewShot") as MockOptimizer,
        patch("backend.train.dspy.Example") as _MockExample,
        patch(
            "builtins.open",
            mock_open(
        ...
        patch("backend.train.configure_lm") as _mock_configure_lm,
    ):
