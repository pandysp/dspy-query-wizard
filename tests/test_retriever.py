...
async def test_colbert_success():
    # We use a unique query to avoid hitting the cache
    query = "unique_query_colbert_success"

    with patch("httpx.Client") as mock_client_cls:
        mock_client = cast(MagicMock, mock_client_cls.return_value)
        mock_client.__enter__.return_value = mock_client  # pyright: ignore[reportAny]

        # Mock ColBERT response
        mock_resp = MagicMock()
        mock_resp.json.return_value = {  # pyright: ignore[reportAny]
            "topk": [
                {"text": "Passage 1", "pid": 1, "score": 10.0},
                {"text": "Passage 2", "pid": 2, "score": 9.0},
            ]
        }
        mock_client.get.return_value = mock_resp  # pyright: ignore[reportAny]
        ...
        call_args = mock_client.get.call_args  # pyright: ignore[reportUnknownMemberType, reportAny]
        assert call_args[0][0] == COLBERT_URL  # pyright: ignore[reportAny]
        assert call_args[1]["params"]["query"] == query  # pyright: ignore[reportAny]
        assert call_args[1]["params"]["k"] == 2  # pyright: ignore[reportAny]


@pytest.mark.asyncio
async def test_fallback_to_wikipedia():
    query = "unique_query_fallback"
    ...
    with patch("httpx.Client") as mock_client_cls:
        ...
        def side_effect(*args: object, **kwargs: object) -> MagicMock:
            ...
            if url == COLBERT_URL:
                ...
                mock_wiki_resp.json.return_value = [  # pyright: ignore[reportAny]
                    query,
                    ["Wiki Title 1"],
                    ["Wiki Desc 1"],
                    ["http://wiki/1"],
        ...
        mock_client.get.side_effect = side_effect  # pyright: ignore[reportAny]
